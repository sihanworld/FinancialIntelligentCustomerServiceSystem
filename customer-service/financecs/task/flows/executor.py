...
from financecs.domain.contexts import SystemCollectInformationContext
from financecs.domain.messages import BotMessage
from financecs.domain.state import DialogueState
from financecs.task.action.runner import ActionRunner, ActionCall
from financecs.task.flows.flows import FlowList
from financecs.task.flows.links import FlowStepStaticLink, FlowStepConditionLink, FlowStepFallbackLink
from financecs.task.flows.steps import FlowStep, StartFlowStep, EndFlowStep, ActionFlowStep, CollectionFlowStep
...
class FlowExecutor:
    ...
    async def execute_flow(self,
        ...
        while True:
            ...
            # 1. 找流程步骤是Action
            action_call = self._advance_flow_until_action(state, flow_list)
    ...
    def _advance_flow_until_action(self,
                                   state: DialogueState,
    ...
    def _try_fill_slots_from_object(self,
        ...
        # 2. 卡片类型和候选槽位的映射（金融对象体系）
        object_type_slots_mapping = {
            "account": ["account_no"],
            "bank_card": ["account_no", "loss_card_no"],
            "transaction": ["transaction_no", "related_business_no"],
        ...
        # 3. 获取候选槽位列表
        expected_slot_names = object_type_slots_mapping.get(state.focused_object.type, [])

        # 4. 当前步骤缺失的槽位命中候选槽位，且业务流程上下文中该槽位还为空时，才利用之前点击的卡片
        if step.slot_name in expected_slot_names and not state.active_task.slots.get(step.slot_name):
            slot_value = state.focused_object.id
            attributes = state.focused_object.attributes or {}
            # 优先使用卡片 attributes 中更精确的字段（例如账户卡片中的 account_no）
            if attributes.get(step.slot_name):
                slot_value = str(attributes[step.slot_name])
            state.set_slots({step.slot_name: slot_value})
